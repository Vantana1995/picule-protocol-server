...
const generalLimiter = rateLimit({
  ...
  legacyHeaders: false,
  handler: (req, res) => {
    const realIP = req.headers["cf-connecting-ip"] || ipKeyGenerator(req);
    logger.warn(`Rate limit exceeded for IP: ${realIP} on ${req.url}`);
    res.status(429).json({
      error: "Too many requests",
      message: "Rate limit exceeded. Maximum 60 requests per minute.",
      retryAfter: 60,
...
const ddosProtection = rateLimit({
  ...
  max: 40, // 40 requests per 30 seconds (aggressive)
  keyGenerator: (req) => req.headers["cf-connecting-ip"] || ipKeyGenerator(req),
  message: {
  ...
  handler: (req, res) => {
    ...
    const warnings = ddosWarnings.get(realIP) || 0;
    ddosWarnings.set(realIP, warnings + 1);

    logger.error(
      `DDoS protection triggered for IP: ${realIP} (Warning ${warnings + 1}/5)`
    );

    if (warnings + 1 >= 5) {
      // 5th warning = permanent ban
      permanentBlacklist.add(realIP);
      ddosWarnings.delete(realIP); // Clean up warnings
      logger.error(
        `IP ${realIP} PERMANENTLY BLACKLISTED after 5 DDoS attempts`
      );
